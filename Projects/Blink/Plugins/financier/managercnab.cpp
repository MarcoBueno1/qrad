#include "managercnab.h"
#include "ui_managercnab.h"
#include "editcnab.h"
...
#define SQL_ITEMS "select id,id from cnab order by id" 

Managercnab::Managercnab(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Managercnab)
{
    ui->setupUi(this);
...
void Managercnab::KeyPressTimeout()
{
...
void Managercnab::StartTimer( QString )
{
...
void Managercnab::Found(QModelIndex)
{
...
void Managercnab::notFound()
{
   ui->lineEditSearch->setStyleSheet(FG_COLOR_NOT_FOUND + BG_COLOR_NOT_FOUND);
  // ui->tableViewSearch->selectRow(0);
}
void Managercnab::TableClicked(QModelIndex currentIndex)
{
    qDebug() << "TableClicked";
    ui->lineEditSearch->setStyleSheet(AUTO_CONFIG_FOCUS);
...
void Managercnab::CurrentChanged(QModelIndex currentIndex)
{
    ui->lineEditSearch->setStyleSheet(AUTO_CONFIG_FOCUS);
    ui->lineEditSearch->setText(currentIndex.sibling(currentIndex.row(),BN_DEFAULT_COLUMN_SEARCH).data().toString());
    ui->lineEditSearch->selectAll();
...
void Managercnab::ShowCurrentInformations( void )
{
    QString strTemp;
...
void Managercnab::LoadTableView()
{
...
void Managercnab::ConfigureTable()
{
...
void Managercnab::keyPressEvent(QKeyEvent *event)
{
...
void Managercnab::MatchNewest(cnab *newest )
{
    DoRefresh();
...
void Managercnab::doEditar()
{
    Editcnab *edt = new Editcnab;
    ...
    cnab *sa = cnab::findByPrimaryKey(nId);
    edt->SetModel(sa);
