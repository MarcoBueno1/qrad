#include "managerlayoutbol.h"
#include "ui_managerlayoutbol.h"
...
#define SQL_ITEMS "select id,id from layoutbol order by id" 

Managerlayoutbol::Managerlayoutbol(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Managerlayoutbol)
{
    ui->setupUi(this);
...
Managerlayoutbol::~Managerlayoutbol()
{
     if( m_keyinterval )
     {
       m_keyinterval->stop();
...
void Managerlayoutbol::KeyPressTimeout()
{
...
void Managerlayoutbol::StartTimer( QString )
{
...
void Managerlayoutbol::Found(QModelIndex)
{
...
void Managerlayoutbol::notFound()
{
   ui->lineEditSearch->setStyleSheet(FG_COLOR_NOT_FOUND + BG_COLOR_NOT_FOUND);
  // ui->tableViewSearch->selectRow(0);
}
void Managerlayoutbol::TableClicked(QModelIndex currentIndex)
{
    qDebug() << "TableClicked";
    ui->lineEditSearch->setStyleSheet(AUTO_CONFIG_FOCUS);
...
void Managerlayoutbol::CurrentChanged(QModelIndex currentIndex)
{
    ui->lineEditSearch->setStyleSheet(AUTO_CONFIG_FOCUS);
    ui->lineEditSearch->setText(currentIndex.sibling(currentIndex.row(),BN_DEFAULT_COLUMN_SEARCH).data().toString());
    ui->lineEditSearch->selectAll();
...
void Managerlayoutbol::ShowCurrentInformations( void )
{
    QString strTemp;
...
void Managerlayoutbol::LoadTableView()
{
...
void Managerlayoutbol::DoRefresh()
{
    ui->tableViewSearch->SetNoEmptySearch( true );
...
void Managerlayoutbol::refreshTable()
{
...
void Managerlayoutbol::ConfigureTable()
{
...
void Managerlayoutbol::keyPressEvent(QKeyEvent *event)
{
...
void Managerlayoutbol::MatchNewest(layoutbol *newest )
{
    DoRefresh();
...
void Managerlayoutbol::doEditar()
{
    Editlayoutbol *edt = new Editlayoutbol;
    ...
    layoutbol *sa = layoutbol::findByPrimaryKey(nId);
    edt->SetModel(sa);
