#include "managerrespemis.h"
#include "ui_managerrespemis.h"
...
#define SQL_ITEMS "select id,id from respemis order by id" 

Managerrespemis::Managerrespemis(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Managerrespemis)
{
    ui->setupUi(this);
...
Managerrespemis::~Managerrespemis()
{
     if( m_keyinterval )
     {
       m_keyinterval->stop();
...
void Managerrespemis::KeyPressTimeout()
{
...
void Managerrespemis::StartTimer( QString )
{
...
void Managerrespemis::Found(QModelIndex)
{
...
void Managerrespemis::notFound()
{
   ui->lineEditSearch->setStyleSheet(FG_COLOR_NOT_FOUND + BG_COLOR_NOT_FOUND);
  // ui->tableViewSearch->selectRow(0);
}
void Managerrespemis::TableClicked(QModelIndex currentIndex)
{
    qDebug() << "TableClicked";
    ui->lineEditSearch->setStyleSheet(AUTO_CONFIG_FOCUS);
...
void Managerrespemis::CurrentChanged(QModelIndex currentIndex)
{
    ui->lineEditSearch->setStyleSheet(AUTO_CONFIG_FOCUS);
    ui->lineEditSearch->setText(currentIndex.sibling(currentIndex.row(),BN_DEFAULT_COLUMN_SEARCH).data().toString());
    ui->lineEditSearch->selectAll();
...
void Managerrespemis::ShowCurrentInformations( void )
{
    QString strTemp;
...
void Managerrespemis::LoadTableView()
{
...
void Managerrespemis::DoRefresh()
{
    ui->tableViewSearch->SetNoEmptySearch( true );
...
void Managerrespemis::refreshTable()
{
...
void Managerrespemis::ConfigureTable()
{
...
void Managerrespemis::keyPressEvent(QKeyEvent *event)
{
...
void Managerrespemis::MatchNewest(respemis *newest )
{
    DoRefresh();
...
void Managerrespemis::doEditar()
{
    Editrespemis *edt = new Editrespemis;
    ...
    respemis *sa = respemis::findByPrimaryKey(nId);
    edt->SetModel(sa);
