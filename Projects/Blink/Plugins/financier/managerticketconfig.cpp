...
#include "ui_managerticketconfig.h"
...
#define SQL_ITEMS "select codigocedente as \"Cedente\", carteira as \"Carteira\", description as \"Descrição\" ,id from ticketconfig order by tipopessoa"

Managerticketconfig::Managerticketconfig(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Managerticketconfig)
{
    ui->setupUi(this);
...
Managerticketconfig::~Managerticketconfig()
{
     if( m_keyinterval )
     {
       m_keyinterval->stop();
...
void Managerticketconfig::KeyPressTimeout()
{
...
void Managerticketconfig::StartTimer( QString )
{
...
void Managerticketconfig::Found(QModelIndex)
{
...
void Managerticketconfig::notFound()
{
   ui->lineEditSearch->setStyleSheet(FG_COLOR_NOT_FOUND + BG_COLOR_NOT_FOUND);
  // ui->tableViewSearch->selectRow(0);
}
void Managerticketconfig::TableClicked(QModelIndex currentIndex)
{
    qDebug() << "TableClicked";
    ui->lineEditSearch->setStyleSheet(AUTO_CONFIG_FOCUS);
...
void Managerticketconfig::CurrentChanged(QModelIndex currentIndex)
{
    ui->lineEditSearch->setStyleSheet(AUTO_CONFIG_FOCUS);
    ui->lineEditSearch->setText(currentIndex.sibling(currentIndex.row(),BN_DEFAULT_COLUMN_SEARCH).data().toString());
    ui->lineEditSearch->selectAll();
...
void Managerticketconfig::ShowCurrentInformations( void )
{
    QString strTemp;
...
void Managerticketconfig::LoadTableView()
{
...
void Managerticketconfig::DoRefresh()
{
    ui->tableViewSearch->SetNoEmptySearch( true );
...
void Managerticketconfig::refreshTable()
{
...
void Managerticketconfig::ConfigureTable()
{
...
void Managerticketconfig::keyPressEvent(QKeyEvent *event)
{
...
void Managerticketconfig::MatchNewest(ticketconfig *newest )
{
    DoRefresh();
...
void Managerticketconfig::doEditar()
{
    Editticketconfig *edt = new Editticketconfig;
    ...
    ticketconfig *sa = ticketconfig::findByPrimaryKey(nId);
    edt->SetModel(sa);
...
void Managerticketconfig::doNovo()
{
    Editticketconfig *edt = new Editticketconfig;
...
void Managerticketconfig::doSair()
{
