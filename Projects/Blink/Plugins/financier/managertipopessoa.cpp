#include "managertipopessoa.h"
#include "ui_managertipopessoa.h"
...
#define SQL_ITEMS "select id,id from tipopessoa order by id" 

Managertipopessoa::Managertipopessoa(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Managertipopessoa)
{
    ui->setupUi(this);
...
Managertipopessoa::~Managertipopessoa()
{
     if( m_keyinterval )
     {
       m_keyinterval->stop();
...
void Managertipopessoa::KeyPressTimeout()
{
...
void Managertipopessoa::StartTimer( QString )
{
...
void Managertipopessoa::Found(QModelIndex)
{
...
void Managertipopessoa::notFound()
{
   ui->lineEditSearch->setStyleSheet(FG_COLOR_NOT_FOUND + BG_COLOR_NOT_FOUND);
  // ui->tableViewSearch->selectRow(0);
}
void Managertipopessoa::TableClicked(QModelIndex currentIndex)
{
    qDebug() << "TableClicked";
    ui->lineEditSearch->setStyleSheet(AUTO_CONFIG_FOCUS);
...
void Managertipopessoa::CurrentChanged(QModelIndex currentIndex)
{
    ui->lineEditSearch->setStyleSheet(AUTO_CONFIG_FOCUS);
    ui->lineEditSearch->setText(currentIndex.sibling(currentIndex.row(),BN_DEFAULT_COLUMN_SEARCH).data().toString());
    ui->lineEditSearch->selectAll();
...
void Managertipopessoa::ShowCurrentInformations( void )
{
    QString strTemp;
...
void Managertipopessoa::LoadTableView()
{
...
void Managertipopessoa::DoRefresh()
{
    ui->tableViewSearch->SetNoEmptySearch( true );
...
void Managertipopessoa::refreshTable()
{
...
void Managertipopessoa::ConfigureTable()
{
...
void Managertipopessoa::keyPressEvent(QKeyEvent *event)
{
...
void Managertipopessoa::MatchNewest(tipopessoa *newest )
{
    DoRefresh();
...
void Managertipopessoa::doEditar()
{
    Edittipopessoa *edt = new Edittipopessoa;
    ...
    tipopessoa *sa = tipopessoa::findByPrimaryKey(nId);
    edt->SetModel(sa);
