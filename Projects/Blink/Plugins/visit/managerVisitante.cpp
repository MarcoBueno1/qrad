...
#define SQL_ITEMS "select * from Visitante order by id"

ManagerVisitante::ManagerVisitante(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ManagerVisitante)
{
    ui->setupUi(this);
...
void ManagerVisitante::KeyPressTimeout()
{
...
void ManagerVisitante::StartTimer( QString )
{
...
void ManagerVisitante::Found(QModelIndex)
{
...
void ManagerVisitante::notFound()
{
   ui->lineEditSearch->setStyleSheet(FG_COLOR_NOT_FOUND + BG_COLOR_NOT_FOUND);
  // ui->tableViewSearch->selectRow(0);
}
void ManagerVisitante::TableClicked(QModelIndex currentIndex)
{
    qDebug() << "TableClicked";
    ui->lineEditSearch->setStyleSheet(AUTO_CONFIG_FOCUS);
...
void ManagerVisitante::CurrentChanged(QModelIndex currentIndex)
{
    ui->lineEditSearch->setStyleSheet(AUTO_CONFIG_FOCUS);
    ui->lineEditSearch->setText(currentIndex.sibling(currentIndex.row(),BN_DEFAULT_COLUMN_SEARCH).data().toString());
    ui->lineEditSearch->selectAll();
...
void ManagerVisitante::ShowCurrentInformations( void )
{
    QString strTemp;
...
void ManagerVisitante::LoadTableView()
{
...
void ManagerVisitante::DoRefresh()
{
    ui->tableViewSearch->SetNoEmptySearch( true );
...
void ManagerVisitante::refreshTable()
{
...
void ManagerVisitante::ConfigureTable()
{
...
void ManagerVisitante::keyPressEvent(QKeyEvent *event)
{
...
void ManagerVisitante::MatchNewest(Visitante *newest )
{
    DoRefresh();
...
void ManagerVisitante::doEditar()
{
    EditVisitante *edt = new EditVisitante;
    ...
    Visitante *sa = Visitante::findByid(nId);
    edt->SetModel(sa);
