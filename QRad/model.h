...
#define MODEL_DECLARE_DOUBLE_ATTRIBUTE(attr)\
    ...
    public: \
        double get##attr(bool bRound = true) { return bRound?dsmRound::PowerRound(m_##attr.value()):m_##attr.value(); }\
        void set##attr(double value, bool bRound = true) { m_##attr.setValue(bRound?dsmRound::PowerRound(value):value); }\
        bool update##attr(double value, bool bRound = true) \
        { \
          m_##attr.setValue(bRound?dsmRound::PowerRound(value):value);\
          QString str = QString("update %1 set %2 = \'%3\' where %4 = \'%5\';") \
                        .arg(tableName()) \
                        .arg(fieldName(#attr)) \
                        .arg(m_##attr.toString().replace("\'", "\'\'"))\
...
#define MODEL_DECLARE_DOUBLE_3_DECIMAL_ATTRIBUTE(attr)\
    protected:\
        ModelAttribute_double m_##attr;\
    public: \
        double get##attr(void) { return dsmRound::PowerRoundAmount(m_##attr.value()); }\
        void set##attr(double value) { m_##attr.setValue(dsmRound::PowerRoundAmount(value)); }\
        bool update##attr(double value) \
        { \
          m_##attr.setValue(dsmRound::PowerRoundAmount(value));\
          QString str = QString("update %1 set %2 = \'%3\' where %4 = \'%5\';") \
                        .arg(tableName()) \
                        .arg(fieldName(#attr)) \
                        .arg(m_##attr.toString().replace("\'", "\'\'"))\
...
#define MODEL_DECLARE_DOUBLE_POSITIVE_ATTRIBUTE(attr)\
    protected:\
        ModelAttribute_double m_##attr;\
    public: \
        double get##attr(void) { return dsmRound::PowerRound(m_##attr.value()); }\
        void set##attr(double value) { m_##attr.setValue(dsmRound::PowerRound(value)); }\
        bool update##attr(double value) \
        { \
          m_##attr.setValue(dsmRound::PowerRound(value));\
          QString str = QString("update %1 set %2 = case when %3 >= 0 then %3 else 0 end where %4 = \'%5\';") \
                        .arg(tableName()) \
                        .arg(fieldName(#attr)) \
                        .arg(m_##attr.toString().replace("\'", "\'\'"))\
