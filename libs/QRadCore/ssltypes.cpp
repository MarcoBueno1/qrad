...
#define LOAD_FUNCTION(ptr, type, name)   \
    functions->ptr = (type)QRad_GetProcAddress(functions->hInstance, name); \
    if (NULL == functions->ptr)  { debug_message("[SSLTYPES] Lib: %s QRad_GetProcAddress(%s) = false L:%d", OPENSSL_LIB_NAME, name, __LINE__); \
    QRad_FreeLibrary(functions->hInstance); return false; }
...
bool SSLFunctions_Initialize(TSSLFunctions* functions)
{
    ...
    functions->hInstance = QRad_LoadLibrary(OPENSSL_LIB_NAME);
...
void SSLFunctions_Terminate(TSSLFunctions* functions)
{
    ...
    QRad_FreeLibrary(functions->hInstance);
}
