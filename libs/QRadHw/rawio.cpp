...
int win32_open(const char * name, unsigned flags, unsigned perm)
{
   ...
   sa.bInheritHandle=TRUE;
	...
	if ((h = CreateFileA(path,
		...
		&sa, OPEN_EXISTING, crflags, 0)) == INVALID_HANDLE_VALUE) {
		long err = GetLastError();
		if (win32_debug)
...
int main( int argc, char *argv[])
{
	    ...
	    int fd = win32_open("/dev/hda", O_RDONLY, 0);
		
  // HANDLE hClientIbold = 0;    // handle do mailslot de transmissao
   

   // hClientIbold= CreateMailslot ( szClientIbold, 0, MaxEspResposta, NULL );
//   hClientIbold= CreateMailslot ( szClientIbold, 0, 0, &sa );
		
		
    if ( fd < 0 )
    {
        fd = win32_open("/dev/sda", O_RDONLY,0);
    }
...
//	int win32_ioctl(int fd, int code, void * arg)
    ...
    if(!win32_ioctl(fd, HDIO_GET_IDENTITY, &id))
    {
       // QString s1;
