...
DSMSecure::DSMSecure()
{
    debug_message("[DSMSecure] -->DSMSecure()..\n");
...
void DSMSecure::cpykey(char *cBuffer, const int *iBuffer, int size)
{
    int i;
    for (i = 0; i < size; i++)
    {
...
QString DSMSecure::publicExponent()
{
    QString output = "";
...
QString DSMSecure::publicModulus()
{
    QString output = "";
    int count = 0;
...
QByteArray  DSMSecure::privateEncrypt(QByteArray content)
{
...
QByteArray  DSMSecure::publicDecrypt(QByteArray encryptedContent)
{
...
QByteArray DSMSecure::encrypt(QByteArray content)
{
    QByteArray encryptedContent;
    unsigned char *pContent = (unsigned char*)content.data();
    unsigned char *buffer = NULL;
...
QByteArray DSMSecure::decrypt(QByteArray encryptedContent)
{
    debug_message("[DSMSecure] -->decrypt()..\n");
...
qint64 DSMSecure::blockSize()
{
    return m_sslFunctions->RSA_size(m_privKey);
}

QByteArray DSMSecure::md5(QByteArray content)
{
    MD5_CTX ctx;
...
QString DSMSecure::strMd5(QByteArray content)
{
    QString result = "";
...
QString DSMSecure::strMd5(QString content)
{
    return strMd5(content.toLatin1());
}

QString DSMSecure::bin2hex(QByteArray content)
{
    BIGNUM* num;
    char *hex;
