...
QSettings::Format DSMSecureSettings::EncryptedInitFormat = QSettings::registerFormat("enc",
                                                                                      encryptedReadFunc,
                                                                                      encryptedWriteFunc,
                                                                                      Qt::CaseSensitive);

DSMSecureSettings::DSMSecureSettings(QObject *parent)
    : QSettings(parent)
{
    setDefaultFormat(DSMSecureSettings::EncryptedInitFormat);
}
...
{
    debug_message("[DSMSecureSettings] -->encryptedReadFunc\n");
    ...
    DSMSecure *secure = new DSMSecure();

    if (!secure->initialize())
    {
        qDebug() << "secure->initialize() failed.";
        delete secure;
        debug_message("[DSMSecureSettings] <--encryptedReadFunc false\n");
        return false;
    }
...
//        device.close();
    ...
    delete secure;
    debug_message("[DSMSecureSettings] <--encryptedReadFunc true\n");
    return true;
}

bool encryptedWriteFunc(QIODevice &device, const QSettings::SettingsMap &map)
{
    debug_message("[DSMSecureSettings] -->encryptedWriteFunc\n");
    ...
    {
        delete secure;
        debug_message("[DSMSecureSettings] <--encryptedWriteFunc false\n");
        return false;
    }
    ...
    if (!device.isOpen())
    {
        debug_message("[DSMSecureSettings] --encryptedWriteFunc Open-- L:%d\n", __LINE__);
        device.open(QIODevice::WriteOnly);
    }

    if( !device.isWritable() )
    {
        debug_message("[DSMSecureSettings] encryptedWriteFunc::fatal error, no writable file L:%d\n", __LINE__);
    }
    device.reset();
    ...
    debug_message("[DSMSecureSettings] <--encryptedWriteFunc true\n");
    return true;
}
